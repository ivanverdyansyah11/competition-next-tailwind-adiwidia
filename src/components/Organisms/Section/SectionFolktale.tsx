...
export default function SectionFolktale() {
  const { category, province, folktale_slug } = useParams<{
    category: string;
    province: string;
  ...
  const folkTaleParam = convertSlug({ slug: folkTale || '' });

  const [items, setItems] = useState<FolktaleRow[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [errorMsg, setErrorMsg] = useState<string>('');

  const getTitle = (row: FolktaleRow) => row.title || row.name || '';
  const getImage = (row: FolktaleRow) =>
    row.media_url || '';
  const getLocation = (row: FolktaleRow) => row.location || '';
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (folktale_slug) {
        query = query.neq('slug', folktale_slug);
      }
  ...
  }, [category, province, folktale_slug]);
      ...
      <div className="section-content">
        ...
        {!loading &&
          ...
          items.map((row) => (
            <CardFolktale
              key={row.id ?? row.slug}
              redirect={`/culture/${category}/detail/${folktale_slug}`}
              image={getImage(row)}
              title={getTitle(row)}
